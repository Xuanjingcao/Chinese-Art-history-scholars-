...
const stats = [
  ...
  { num: schoolCoverageCount, label: '学校覆盖' },
...
export default function StatsBar() {
  return (
    <div
      className="relative z-10 mx-auto mb-4 grid w-[calc(100%-1.5rem)] max-w-[960px] grid-cols-3 gap-y-3 px-2 py-3 md:mb-5 md:w-[calc(100%-3rem)] md:px-5 md:py-4"
      style={{
    ...
    >
      {stats.map((stat, i) => (
        <div className="relative flex min-w-0 items-center justify-center" key={i}>
          {i > 0 && (
            <div
              className="absolute left-0 top-1/2 h-8 -translate-y-1/2"
