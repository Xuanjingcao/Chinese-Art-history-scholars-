  ...
  return (
    <main className="mx-auto max-w-[760px] px-3 pb-24 pt-4 md:px-6 md:pt-8">
      ...
      <div className="mt-4 space-y-5 rounded-2xl p-4 md:p-6" style={{ backgroundColor: 'rgba(255,253,248,0.72)', border: '1px solid rgba(92,64,48,0.1)' }}>
        ...
        <CommunityImageGrid images={draft.images} coverImageId={draft.coverImageId} onChange={({ images, coverImageId }) => setDraft((current) => ({ ...current, images, coverImageId }))} />
        {draft.images.length > 0 ? <p className="font-kai text-[11px]" style={{ color: '#7a6c5d' }}>发布前会预览第一张封面图。</p> : null}
